<script>
    export default {
      methods: {
        ...
        //Create Entity from a modal component
        createEntityRef (entity, isModal, entity_table, refs, is_tab = false) {
          this.$refs[refs].forms[entity].isSaving = true
          this.$http.post('api/' + entity, this.$refs[refs].forms[entity].fields,{
             headers: {
                    Authorization: 'Bearer ' + localStorage.getItem('token')
                }
          })
          .then((response) => {  
            ...
            console.log(this.tables[entity_table])
            this.tables[entity_table].items.unshift(response.data.data)
            this.paginations[entity_table].totalRows++

            if(isModal){
              this.$refs[refs].showModalEntry = false
            }
            else{
              this.$refs[refs].showEntry = false
            }
          }).catch(error => {
            this.$refs[refs].forms[entity].isSaving = false
            if (!error.response) return
        ...
        createOptionsEntityRef (entity, isModal, entity_table, from_entity, entity_id, refs) {
          this.$http.post('api/' + entity, this.$refs[refs].forms[entity].fields,{
             headers: {
                    Authorization: 'Bearer ' + localStorage.getItem('token')
                }
          ...
          .then((response) => {  
            ...
            for (var key in errors) {
              ...
              if(a == 0){
                this.$refs[refs].focusElement(key)
                this.$notify({
                  type: 'error',
                  group: 'notification',
                  title: 'Error!',
        ...
        //Update entity from a modal component
        updateEntityRef (entity, entity_id, isModal, row, refs, is_tab = false) {
          this.$refs[refs].forms[entity].isSaving = true

          this.$http.put('api/' + entity + '/' + this.$refs[refs].forms[entity].fields[entity_id], this.$refs[refs].forms[entity].fields ,{
              headers: {
                      Authorization: 'Bearer ' + localStorage.getItem('token')
                  }
            })
        ...
        fillEntityForm (entity, id, entry) {
